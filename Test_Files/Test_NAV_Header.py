...
def show(name, img):
    ...
    plt.imshow(img, cmap='gray'), plt.show(block=False), plt.pause(3), plt.close()
...
def cntr_head_adjust(cntr, company_list):
    ...
    ## If there is no header it should be reverse
    if len(cntr_head) == 0:
        return ""
...
def gray_black(gray, gr_bl_constant,k):
...
def visualize_possible_cntrs(height, width, channel, matched_result, black_copy):
    ...
    try:
        ...
        for index in range(2, len(list_x)-1):
...
def erosion_detect(img_cropped, company_list, iteration):
    ...
    # This case should be "reverse"  ex) OOCU7925148-1
    if len(chars) == 0:
        return result_char, img_cropped, chars
...
def main(img, aim, gr_bl_constant, result_possibility,company_list, k):
    reverse = False
    ...
    #### Convert Img to Black&White ####
    black = gray_black(gray, gr_bl_constant, k)
    ...
    def find_chars(contour_list):
        ...
        for d1 in contour_list:
            ...
            for d2 in contour_list:
                ...
                and angle_diff < MAX_ANGLE_DIFF and height_diff < MAX_HEIGHT_DIFF:
                    matched_contours_idx.append(d2['idx'])
                # List which should be deleted
                elif height_diff >= MAX_HEIGHT_DIFF:
                    unmatched_result_idx.append(d2['idx'])
    ...
    if output == 0 and switch_button == False:
        ...
        return main(img2, aim2, gr_bl_constant_reverse, True, company_list, k)
