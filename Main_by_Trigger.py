...
def gray_black(gray, gr_bl_constant, gr_bl_constant_reverse):
    #### Convert Img to Black&White ####
    # (thresh, blackAndWhiteImage) = cv2.threshold(gray, 100, 255, cv2.THRESH_BINARY_INV)  # CKOUT
...
def prepare_data(height, width, channel, contours, h_max, h_min):
...
def find_chars(contour_list, possible_contours, MAX_DIAG_MULTIPLYER=10, MAX_ANGLE_DIFF=8.0, MIN_N_MATCHED=5):
    matched_result_idx = []
...
def main(img, gr_bl_constant, gr_bl_constant_reverse, h_max, h_min, result_possibility, company_list):
    ...
    #### Convert Img to Black&White ####
    black = gray_black(gray, gr_bl_constant, gr_bl_constant_reverse)
    ...
    #### Prepare Data ####
    contours_dict, temp_result2 = prepare_data(height, width, channel, contours, h_max, h_min)
    ...
    if output == 0 and switch_button == False:  # Reverse option
